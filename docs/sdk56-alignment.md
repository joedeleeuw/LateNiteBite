...
| 4 | expo-doctor: SDK-native package versions | **Adopt now** | Removed the dependency-validation exclude list and aligned SDK-coupled packages with Expo 56's expected set: `react-native-gesture-handler@2.31.x`, `react-native-reanimated@4.3.1`, `react-native-safe-area-context@5.7.x`, `react-native-svg@15.15.4`, and `react-native-worklets@0.8.3`. These live in the `expo56` pnpm catalog. |
...
- **Adopted now:** 4 (AbortSignal.timeout, CLAUDE.md + AGENTS.md, app.json schema fix, Expo 56 native package pins)
...
✖ Check that packages match versions required by installed Expo SDK
  ...
  → Fixed: removed the validation exclude list and aligned the Expo 56 native
    package pins in `pnpm-workspace.yaml`.
```
...
`xcode@3.0.1` is the latest stable `xcode` package and is pulled through Expo config tooling, but its `uuid@^7.0.3` dependency is deprecated. `pnpm-workspace.yaml` overrides only `xcode>uuid` to `11.1.1`, the supported CommonJS line. Verified with `xcode.project(...).generateUuid()` so the override exercises the exact call site.
...
| `package.json` | Removed `expo.install.exclude`; SDK-coupled packages use the `expo56` catalog |
| `pnpm-workspace.yaml` | Moved React Native SDK packages into the `expo56` catalog with Expo-compatible pins; overrides `xcode>uuid` to the supported CommonJS line |
